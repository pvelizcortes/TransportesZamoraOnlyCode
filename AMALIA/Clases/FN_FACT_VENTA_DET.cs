...
namespace AMALIAFW
{
    public class OBJ_FACT_VENTA_DET
    {
    ...
    public static class FN_FACT_VENTA_DET
    {
        public static string connex = ConfigurationManager.ConnectionStrings["default"].ToString();

        // CAMBIAR AQUI //
        // **************************************** **************************************** **************************************** //       
        private static string nombre_tabla = "FACT_VENTA_DET";
        private static string nombre_llave = "ID_VENTA_DET";
        private static string value_insert = "id_fact_venta, nombre_archivo, nombre_archivo_server, extension, url_doc, path_local, fecha_subida";
        private static string value_insert2 = "@id_fact_venta, @nombre_archivo, @nombre_archivo_server, @extension, @url_doc, @path_local, @fecha_subida";
        private static string value_update = "id_fact_venta = @id_fact_venta, " +
        ...
        public static void INSERT(ref OBJ_FACT_VENTA_DET objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(sql, conn))
                    {
                        // CAMBIAR AQUI //
                        // **************************************** **************************************** **************************************** // 
                        cmd.Parameters.AddWithValue("@id_fact_venta", objeto.id_fact_venta);
        ...
        public static void UPDATE(ref OBJ_FACT_VENTA_DET objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(sql, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@" + nombre_llave, objeto.ID_VENTA_DET);
                        // **************************************** **************************************** **************************************** //                       
        ...
        public static void DELETE(ref OBJ_FACT_VENTA_DET objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
        ...
        public static void LLENAOBJETO(ref OBJ_FACT_VENTA_DET objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
        ...
        public static void PREPARAOBJETO(ref OBJ_FACT_VENTA_DET objeto)
        {
            try
            {
                // **************************************** **************************************** **************************************** //  
                objeto.id_fact_venta = 0;
