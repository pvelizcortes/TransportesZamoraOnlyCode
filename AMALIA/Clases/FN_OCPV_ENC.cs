...
namespace AMALIAFW
{
    ...
    public static class FN_OCPV_ENC
    {
        ...
        public static void INSERT(ref OBJ_OCPV_ENC objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
        ...
        public static void UPDATE(ref OBJ_OCPV_ENC objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
        ...
        public static void DELETE(ref OBJ_OCPV_ENC objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
        ...
        public static void LLENAOBJETO(ref OBJ_OCPV_ENC objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
        ...
        public static void PREPARAOBJETO(ref OBJ_OCPV_ENC objeto)
        {
            try
            {
                // **************************************** **************************************** **************************************** //  
        ...
        public static int GetCorrelativo()
        {
            ...
            int scalar = int.Parse(db.Scalar("select (ISNULL(MAX(oc_agro),0) + 1) from correlativos").ToString());
            return scalar;
        }

        public static void GetTotales(ref OBJ_OCPV_ENC objeto)
        {
            try
            {
                DBUtil db = new DBUtil();
                DataTable dt = new DataTable();
                dt = db.consultar("select isnull(sum(neto),0) as 'neto', isnull(sum(iva),0) as 'iva', isnull(sum(total),0) as 'total' from ocpv_detalle where id_oc = " + objeto.ID_OC);
                FN_OCPV_ENC.LLENAOBJETO(ref objeto);
                if (objeto._respok)
                {
        ...
        public static DataTable getDetalleOC(int IDOC)
        {
            ...
            {
                conn.Open();
                string sql = @"SELECT *  from v_pv_ordenes_compra_detalle where id_oc = " + IDOC;
                SqlCommand cmd = new SqlCommand(sql, conn);
                SqlDataAdapter ap = new SqlDataAdapter(cmd);
                ap.Fill(dt);
            }
