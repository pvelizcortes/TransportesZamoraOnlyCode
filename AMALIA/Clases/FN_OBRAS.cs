...
namespace AMALIAFW
{
    ...
    public static class FN_OBRAS
    {
        public static string connex = ConfigurationManager.ConnectionStrings["default"].ToString();

        // CAMBIAR AQUI //
        // **************************************** **************************************** **************************************** //       
        private static string nombre_tabla = "obras";
        private static string nombre_llave = "ID_OBRA";
        private static string value_insert = " cliente, obra ";
        private static string value_insert2 = " @cliente, @obra ";
        private static string value_update =
        " cliente = @cliente, " +   
        " obra = @obra " + // <----------- EL ULTIMO SIN COMA PLS <3
        ...
        public static void INSERT(ref OBJ_OBRAS objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(sql, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@obra", objeto.obra);
                      

                        // **************************************** **************************************** **************************************** //  
                        int scope = Convert.ToInt32(cmd.ExecuteScalar());
                        // AQUI TAMBIEN
                        objeto.ID_OBRA = scope;
        ...
        public static void UPDATE(ref OBJ_OBRAS objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(sql, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@" + nombre_llave, objeto.ID_OBRA);
                        // **************************************** **************************************** **************************************** //                       
        ...
        public static void DELETE(ref OBJ_OBRAS objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
        ...
        public static void LLENAOBJETO(ref OBJ_OBRAS objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
                {
                    ...
                    {
                        // patente, num_chasis, num_motor, marca, ano, carga, activo
                        objeto.cliente = reader["cliente"].ToString();
                        objeto.obra = reader["obra"].ToString();                       
                        // **************************************** **************************************** **************************************** //    
                        objeto._respok = true;
        ...
        public static void PREPARAOBJETO(ref OBJ_OBRAS objeto)
        {
            try
            {
                // **************************************** **************************************** **************************************** //  
