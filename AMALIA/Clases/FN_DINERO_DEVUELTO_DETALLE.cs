...
namespace AMALIAFW
{
    public class OBJ_DINERO_DEVUELTO_DETALLE
    {
        // PK AUTONUMERADA & PROPIOS DE LA TABLA
        public int ID_DETALLE_DINERO_DEVUELTO { get; set; }
    ...
    public static class FN_DINERO_DEVUELTO_DETALLE
    {
        public static string connex = ConfigurationManager.ConnectionStrings["default"].ToString();

        // CAMBIAR AQUI //
        // **************************************** **************************************** **************************************** //       
        private static string nombre_tabla = "DINERO_DEVUELTO_DETALLE";
        private static string nombre_llave = "ID_DETALLE_DINERO_DEVUELTO";
        private static string value_insert = "id_dinero_devuelto, num_viaje, fecha_viaje, id_conductor, nombre_conductor, tipo, RP, valor, monto_depositado, comentario, estado, usuario_admin, comentario_admin, fecha_admin";
        private static string value_insert2 = "@id_dinero_devuelto, @num_viaje, @fecha_viaje, @id_conductor, @nombre_conductor, @tipo, @RP, @valor, @monto_depositado, @comentario, @estado, @usuario_admin, @comentario_admin, @fecha_admin";
        private static string value_update = "id_dinero_devuelto = @id_dinero_devuelto, " +
        "num_viaje = @num_viaje, " +
        ...
        public static void INSERT(ref OBJ_DINERO_DEVUELTO_DETALLE objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(sql, conn))
                    {
                        // CAMBIAR AQUI //
                        // **************************************** **************************************** **************************************** // 
                        cmd.Parameters.AddWithValue("@id_dinero_devuelto", objeto.id_dinero_devuelto);
                        ...
                        // AQUI TAMBIEN
                        objeto.ID_DETALLE_DINERO_DEVUELTO = scope;
        ...
        public static void UPDATE(ref OBJ_DINERO_DEVUELTO_DETALLE objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(sql, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@" + nombre_llave, objeto.ID_DETALLE_DINERO_DEVUELTO);
                        // **************************************** **************************************** **************************************** //                       
        ...
        public static void DELETE(ref OBJ_DINERO_DEVUELTO_DETALLE objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
        ...
        public static void LLENAOBJETO(ref OBJ_DINERO_DEVUELTO_DETALLE objeto)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connex))
                {
                    ...
                    if (reader.Read())
                    {
                        objeto.id_dinero_devuelto = int.Parse(reader["id_dinero_devuelto"].ToString());
        ...
        public static void PREPARAOBJETO(ref OBJ_DINERO_DEVUELTO_DETALLE objeto)
        {
            try
            {
                // **************************************** **************************************** **************************************** //  
                objeto.id_dinero_devuelto = 0;
                objeto.num_viaje = 0;
